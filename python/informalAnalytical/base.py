...
class Base:
    ...
    def ΓsCap(self, τ, θ, t = None):
        ...
        Above it Θh and BSteadyState's `inner` are negative, so a fractional power returns NaN and a
        bracketing root finder dies rather than reporting a bad bracket. A hard feasibility limit: the
        steady-state root is always strictly below it. Scales with α/(1-α), so a constant upper bound that
        was safe at α = 0.43 is inside the infeasible region at α = 0.35 for τ large enough -- see
        model.steadyState_CRRA_bounds and notes/crossCuttingFindings.md #7. """
