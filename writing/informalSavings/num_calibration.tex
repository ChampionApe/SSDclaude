...
\eqref{\refeq:calibration:KY} carries the factor $n_{\text{yr}}=30$, the number of years in a model period, and that factor is the whole content of the target. A period's output $Y_{t_0}$ is thirty years of production while $s_{t_0-1}/\nu_{t_0}$ is a stock, so the model's own ratio $K_{t_0}/Y_{t_0}$ is a \emph{thirtieth} of the capital--output ratio that national accounts report; without $n_{\text{yr}}$ the two are not the same object. The savings rate
\begin{align}
...
\smallskip\noindent\emph{Residual.} Form the gap for \eqref{\refeq:calibration:tau} as a level difference and the other three as relative differences. Here the scaling is not immaterial for the first row: $K/Y\approx 3.6$ against $\tau = 0.125$, so a level gap on \eqref{\refeq:calibration:KY} would be an order of magnitude larger than the tax gap and the root finder would buy accuracy in the capital--output ratio with accuracy in the tax rate. Relative, the two are both $O(0.1)$. For \eqref{\refeq:calibration:eta0} and \eqref{\refeq:calibration:X0} the choice is close to immaterial --- at the calibrated point $\eta_0 = 0.326$ and $X_0 = 0.414$ --- and a relative gap is in any case the natural form for a self-consistency condition.
