...
| `Tables/UK_ESC_Calibration.tex` | `results/esc/escCountry.csv` — the UK's own wedge, ρ = 1 only; the US-wedge choice goes in the note |
| `Figs/US_ESC_overview.pdf` | `escExperiments.csv` — 2×2 dumbbells (design, τ, `s/Y`, workweek): open marker = `θ` pinned, filled = chosen, per (scenario, ρ) |

**The French leisure row is no longer printed anywhere** (2026-09-11: a pure rescaling of `X_i` that moves
hours alone, judged uninformative). It still runs, still lands in every csv, and is still *inside* the
all-characteristics rows (which need France's level of `X` to land on France's own `(η, X)`); only the
readers dropped it. `frBoth` and `frAll` in the ESC figure/tables are therefore the two composites left.
...
- **One savings-rate unit, `s/Y`, in every table and figure of both arms (2026-09-08).** The US shock csv
  ...
  leisure row in the csv must read `0.00 p.p.` at every ρ, which is the check that the differencing is
  against the right baseline (it is no longer printed, but the check still holds on the csv). The ESC
  tables difference against the *endogenous* baseline, so an exogenous pure-scale row would carry the
  0.01–0.02 p.p. gap between the two baselines rather than an effect.
...
- **The two variants check each other.** Run the same shock set under both and compare row by row:
  ...
  **Leisure (csv only, no longer printed) is the one that must differ in the WORKWEEK and not in τ**: it
  is a pure scale, so τ and the savings rate are pinned at baseline in *both* variants (they agree to
  2.3e-5, the solver's own tolerance) and the whole variant effect lands on hours, because
  `Xbar_FR/Xbar_US` is 1.520 under vector `X` and 1.761 under common `X`. Checking that row in τ would
  report a pass on a quantity that cannot move.
