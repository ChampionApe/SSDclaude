...
\end{align}
where $\bm{y}_i^{\eta} = \text{Eigvec}(\bm{z}_i^{\eta}\times \bm{\gamma}_i^{\prime})$ and $\bm{y}_i^{x} = \text{Eigvec}(\bm{z}_i^{x}\times \bm{\gamma}_i^{\prime})$. The division by $\bm{y}_i^{\eta}\cdot\bm{\gamma}_i$ in \eqref{\refeq:calibration:etai} is what enforces $\Gamma_h = 1$. The scale of $\bm{y}^x_i$ is a second and separate choice: by \eqref{\refeq:hoursUnit} it \emph{is} the hours unit $\mu = \sum_i\gamma_iy_i^x$, and relative-hours data cannot speak to it. We spend it the same way, normalizing $\bm{\gamma}_i\cdot\bm{y}^x_i = 1$, so that by \eqref{\refeq:avgHours} the average workweek $\overline{h}_{t_0}$ and the aggregate $h_{t_0}$ are counted in the same unit. This is a units convention and nothing more --- the equilibrium is invariant to it --- but it is one convention rather than one per calibration, which is what makes the reported $\overline{h}$ and $X_i$ comparable across countries.
