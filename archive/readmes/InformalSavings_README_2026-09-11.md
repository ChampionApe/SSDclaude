...
- **The formal `(η_i, X_i)` carry TWO normalizations, and the informal calibration needs both**
  (docs eq:calibration:yNorm): `Γ_h = 1` and `∑γ_i(η_i/X_i)^ξ = 1`, the second being the hours unit. The
  equilibrium is invariant to it, and so is everything a test of `τ`, `s` or `β` can see. But
  `calibrationη0/X0` read `h_t` as average formal hours, which is true only under it. Without it, `X0`
  came out ~20× too low and informal hours at twice their target, while every other number was
  unchanged. The data `z_j` must likewise be relative to the γ-weighted FORMAL mean (eq calibration:z,
  built in `test.py`). `test_calibration.py` checks both on the solved path.
