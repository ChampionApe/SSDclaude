...
**The income row carries France's `η` *profile* at the US productivity *level*** (`shocks.ηLevel`, since
2026-09-11). The model is invariant to the joint scale `(η, X) → (cη, cX)`, which is what `Γ_h = 1`
normalises away; but `η → cη` at *fixed* `X` is not a normalisation — hours depend on `η_i/X_i`, so it
moves `h̄` by `c^ξ` while τ, `s/Y` and `R` stay put to every digit. France's calibrated `η_i` carries the
level `Γ_h = 1` fixes at *France's* `X`, under common `X` a factor `(X_FR/X_US)^{ξ/(1+ξ)}` ≈ 1.14 that is
France's leisure preference showing up as a productivity advantage. Swapped in raw it put the income
row's workweek 4% too high under common `X` (41.97 against 40.36 hours at ρ = 1); under vector `X` the
raw level comes from the unit-norm eigenvector and happened to land at `Γ_h` = 0.998, so those numbers
barely move. The shock now rescales `η_FR` by `c = Γ_h(η_FR, X_US)^{-1/(1+ξ)}` so `Γ_h = 1` holds on the
shocked model as on the baseline, and **the leisure row scales `X` by `c·X̄_FR/X̄_US`** (33.24 → 34.57
hours under common `X`), so the two rows still compose to France's own `(η, X)` up to the joint scale:
`frAll` and France's own row are unchanged, only the split between the two single-characteristic rows
moved. A residual convention remains — "same level" could equally mean `∑γ_i η_i` fixed — worth 0.5% on
the workweek against the 4% at stake. Asserted in `test_esc.py`.
