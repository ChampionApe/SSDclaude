...
\end{align*}
and, for a crossing $\tau^{\star}$ located inside the cell $\left[\tau^{(k)},\tau^{(k+1)}\right]$, by adding the triangle $\frac{1}{2}z\left(\tau^{(k)}\right)\left(\tau^{\star}-\tau^{(k)}\right)$. Setting $\tau = \arg\max_{\tau\in\mathcal{C}}\hat{\mathcal{V}}(\tau)$ resolves multiplicity and corner solutions under a single criterion: $l$ and $u$ compete with the interior crossings on the same footing, so the rule subsumes the boundary check rather than supplementing it. The accuracy demanded of \eqref{\refeq:objectiveProfile} is only that it rank well-separated candidates correctly, which is a far weaker requirement than the accuracy demanded of the located roots themselves. One restriction: the integration is taken over the interior grid $\mathcal{T}$ only, never over the outer nodes of $\tilde{\mathcal{T}}$ --- outside $[l,u]$ the penalised residual $h$ is an artificial construction, not $d\mathcal{W}/d\tau$, and integrating it would be meaningless. When a feasibility mask excludes part of $\mathcal{T}$ (as it will in \cref{\refsec:PEE}), the rule is applied to the feasible sub-grid, with $l$ and $u$ replaced by its endpoints.

\smalltitle{Root problems that are not maximisations.}
Not every root problem below is a first order condition. The state fixed point of \cref{\refsec:PEE} is a self-consistency condition in which \emph{any} crossing is a solution; there, the direction filter and the objective integration above are deliberately not applied. Keeping the two kinds of problem distinct matters, because the safeguards appropriate to one are errors in the other. The design choice of \cref{\refsec:ESC} sits at the opposite end and is not run through this machinery either, for its own reason given there: its objective is evaluated and maximised directly, because whether that choice is interior is itself the question being asked.
