...
\end{align*}
where we use the shorthand $d\tau_{t+1}/d\tau_t = \frac{d \tau^{t+1}}{d (s_t^0/s_t)}\frac{d (s_t^0/s_t)}{d\tau_t}$ to define the total effect of a change in the tax rate on future taxes. In section \ref{sec:numerical}, we outline how to compute the politico-economic derivatives in the numerical application.
